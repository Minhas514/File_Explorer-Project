...
def GetFileName(event):
    ...
    print("\nDouble-left clic :\nx :",event.x,"\ny :",event.y,"\nRow :",X,"\nColumn :",Y,"\n")
    FileName=Grid[X][Y]
    isaFolder=DirGrid[X][Y]
    OpenFile(CurrentDir+"/"+FileName,isaFolder)

def OpenFile(FileName,isFolder, Back=False):
    ...
    if isFolder:
        ...
        subprocess.call(["xdg-open",FileName])   #PROCESS USED HERE TO OPEN FILE DEFAULT DIRECTORY in linux
    elif system=="Windows":
        os.startfile(FileName)


win1=Tk()
win1.title("Linux / Windows File Explorer by Ali-Waris-Umar")
...
def DeleteFile(FileName): #Main delete confirmation window
    ...
    if RememberChoice.get()==0:
        ...
        YesButton=Button(WinDeleteConfirm,bg="black",fg="white",highlightthickness=0,text="YES",command=lambda:ConfirmDelete(CurrentDir,FileName))
...
RightClic.add_command(label="New folder",command=lambda:NewDir(CurrentDir))       #lambda is annoymous function can be used inline.Python lambda functions, also known as anonymous functions, are inline functions that do not have a name.
...
RootText=can2.create_text(usedWidth/12,60,text="Root",font=('Ubuntu', 20, 'bold'),anchor=CENTER,fill="white" )
...
DesktopText=can2.create_text(usedWidth/12,90,text="-----------------------------------------", anchor=CENTER,fill="white")
MediaText=can2.create_text(usedWidth/12,120,text="Media",font=('Ubuntu', 20, 'bold'),anchor=CENTER,fill="white")
...
DesktopText=can2.create_text(usedWidth/12,150,text="-----------------------------------------", anchor=CENTER,fill="white")
DownloadsText=can2.create_text(usedWidth/12,180,text="Downloads",font=('Ubuntu', 20, 'bold'),anchor=CENTER,fill="white")
...
DesktopText=can2.create_text(usedWidth/12,210,text="-----------------------------------------", anchor=CENTER,fill="white")
DesktopText=can2.create_text(usedWidth/12,240,text="Desktop",font=('Ubuntu', 20, 'bold'), anchor=CENTER,fill="white")
can2.tag_bind(DesktopText,"<ButtonPress-1>",DesktopFunction)

DesktopText=can2.create_text(usedWidth/12,370,text="Submitted to: ",font=('Ubuntu', 19, 'bold'), anchor=CENTER,fill="white")
DesktopText=can2.create_text(usedWidth/12,410,text="Sir Kamran Rasheed",font=('Ubuntu', 12, 'bold'), anchor=CENTER,fill="white")
DesktopText=can2.create_text(usedWidth/12,440,text="And",font=('Ubuntu', 12, 'bold'), anchor=CENTER,fill="white")
DesktopText=can2.create_text(usedWidth/12,470,text="Sir Shakeel Ahmed",font=('Ubuntu', 12, 'bold'), anchor=CENTER,fill="white")
...
but3=Button(can3,image=icon_forward,width=70,height=42,bd=0,bg="black",activebackground="gray33",command=GoForwards)
    ...
    for i in lstDir:
        ...
        if currentWidthIconPlacement<maxWidthIconPlacement:
            ...
            if extFile==".txt" and hidden==0:
                ...
                icon=can1.create_image(currentWidthIconPlacement+37.5,currentHeightIconPlacement+37.5,image=icon9_hid)
                lab1=can1.create_text(currentWidthIconPlacement+37.5,currentHeightIconPlacement+87.5,text=i2,fill="white",width=75,justify=CENTER)
                IsDirectory=False
